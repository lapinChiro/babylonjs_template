DROP TYPE IF EXISTS type_generated_set_delete_memos CASCADE;
CREATE TYPE type_generated_set_delete_memos AS (
  uuid UUID
);
...
-- 例外
--   なし
CREATE OR REPLACE FUNCTION generated_set_delete_memos (
  p_uuid UUID DEFAULT NULL
  ,p_now TIMESTAMPTZ DEFAULT NULL
  ,p_pg TEXT DEFAULT NULL
  ,p_operator_uuid UUID DEFAULT NULL
) RETURNS SETOF type_generated_set_delete_memos AS $FUNCTION$
DECLARE
  w_now TIMESTAMPTZ := COALESCE(p_now, NOW());
  w_pg TEXT := COALESCE(p_pg, 'generated_set_delete_memos');
...
BEGIN
  ...
  PERFORM
    dyn_set_save_garbage(
      ...
      ,p_src_table_name := 'memos'
      ,p_dst_table_name := 'memos'
      ,p_now := w_now
      ,p_sp := w_pg
