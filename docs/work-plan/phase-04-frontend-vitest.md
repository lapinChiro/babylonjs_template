...
- [ ] **Step 2: `apps/frontend/vitest.config.ts` を作成**
...
- [ ] **Step 3: `apps/frontend/tsconfig.vitest.json` を作成し、references に追加**
...
- [ ] **Step 4: scripts を追加(`apps/frontend/package.json`)**
...
- [ ] **Step 2: 実行して意図どおりのテストになっていることを確認**
...
- [ ] **Step 3: services のテストを 1 本追加(`src/services/items.service.test.ts`)**
...
- [ ] **Step 4: 型検査・lint がテストを対象に含むことを確認**
...
- [ ] **Step 5: コミット** `feat(frontend): vitest + happy-dom を導入し stores/services のシードテストを追加`

## 受入基準

- [ ] `npm test` が PASS(store 3 ケース+ service 3 ケース以上)
- [ ] テストコードが type-check / lint の対象に入っている(わざと型エラーをテストに入れて検出されることを 1 度確認し、戻す)
- [ ] build / knip 回帰なし
