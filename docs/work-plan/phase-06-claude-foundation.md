...
- [ ] **Step 1: 以下の内容で作成**(移植元との差分: bastion スクリプト削除・psql は compose のデフォルト認証に適合・Skill allow を移植後の skill 一覧に一致・`mcp__drawio` 削除)
...
- [ ] **Step 1: 以下の内容で作成**(移植元の構成を踏襲し、内容を本プロジェクトに全面適合。ルール索引の一覧は Phase 7 で作る rule のサマリと一致させること)
...
- [ ] **Step 2: コピー後に 3 ファイルを Read し、移植元固有の記述(あれば)を確認・修正する**(coder.md は事前調査でプロジェクト非依存と確認済み。model 固定は維持: coder/start_dev = claude-opus-4-8、create-next-prd = claude-fable-5)
...
- [ ] `CLAUDE.md` / `.claude/settings.json` / `.claude/agents/coder.md` / `.claude/commands/{start_dev,create-next-prd}.md` が存在
- [ ] `.claude/skills/modern-web-guidance/SKILL.md` が symlink 経由で読める
- [ ] CLAUDE.md 記載のコマンド(`docker compose up --build -d`、各 npm script)が実在する(`package.json` と突合)
- [ ] 新しいセッションを起動すると CLAUDE.md が読み込まれ、日本語応答・ワークフロー指示が効いている(簡易確認)
