# Phase 6: Claude Code 基盤(settings.json / CLAUDE.md / coder / commands / modern-web-guidance)

> 前提: `docs/work-plan/README.md` と Phase 1〜5 完了。**このフェーズ以降が「設定移植」本体**。移植元は `claude_settings/`(読み取り専用)。

**ゴール:** Claude Code 設定の土台(root `CLAUDE.md`、`.claude/settings.json`、`coder` エージェント、コマンド 2 つ、vendored skill)を構築する。

注意: この時点で CLAUDE.md が参照する rules / skills はまだ存在しない(Phase 7〜10 で作成)。参照整合は Phase 10 で最終検証するので、このフェーズでは気にせず最終形を書く。

### Task 1: `.claude/settings.json`

**Files:**
- Create: `.claude/settings.json`

- [x] **Step 1: 以下の内容で作成**(移植元との差分: bastion スクリプト削除・psql は compose のデフォルト認証に適合・Skill allow を移植後の skill 一覧に一致・`mcp__drawio` 削除)
...
```json
{
  ...
  "permissions": {
    ...
    "allow": [
      ...
      "Skill(test-design)",
      "Skill(quality-check)",
      "Skill(refactoring-check)",
      "Skill(review-dev)",
      "Skill(review-prd)",
      "Skill(check-local)",
      "Skill(todo-audit)",
      "Skill(prd-completion)",
      "Skill(backlog-management)",
      "Skill(backlog-replenishment)",
      "Skill(todo-replenishment)",
...
(移植元の `enableAllProjectMcpServers` は本プロジェクトに `.mcp.json` が無いため入れない)

### Task 2: root `CLAUDE.md`

**Files:**
- Create: `CLAUDE.md`(リポジトリ root)

- [x] **Step 1: 以下の内容で作成**(移植元の構成を踏襲し、内容を本プロジェクトに全面適合。ルール索引の一覧は Phase 7 で作る rule のサマリと一致させること)
...
### Task 3: coder エージェントとコマンド

**Files:**
- Create: `.claude/agents/coder.md`(`claude_settings/.claude/agents/coder.md` をそのままコピー)
- Create: `.claude/commands/start_dev.md`(コピー。内容の `plan.md` 参照はそのまま有効)
- Create: `.claude/commands/create-next-prd.md`(コピー)

- [x] **Step 1: 3 ファイルをコピーする**

```bash
mkdir -p .claude/agents .claude/commands
cp claude_settings/.claude/agents/coder.md .claude/agents/
cp claude_settings/.claude/commands/start_dev.md .claude/commands/
cp claude_settings/.claude/commands/create-next-prd.md .claude/commands/
```

- [x] **Step 2: コピー後に 3 ファイルを Read し、移植元固有の記述(あれば)を確認・修正する**(coder.md は事前調査でプロジェクト非依存と確認済み。model 固定は維持: coder/start_dev = claude-opus-4-8、create-next-prd = claude-fable-5)

### Task 4: modern-web-guidance(vendored)

**Files:**
- Create: `.agents/skills/modern-web-guidance/`(丸ごとコピー、~140 ファイル)
- Create: `.claude/skills/modern-web-guidance`(symlink)

- [x] **Step 1: コピーと symlink**

```bash
mkdir -p .agents/skills .claude/skills
cp -r claude_settings/.agents/skills/modern-web-guidance .agents/skills/
ln -s ../../.agents/skills/modern-web-guidance .claude/skills/modern-web-guidance
ls -la .claude/skills/modern-web-guidance/  # SKILL.md が見えること
```

(vendored コーパスの内容は編集しない。移植元の symlink 構造を `ls -la claude_settings/.claude/skills/` で確認し、相対パスを同じ形に合わせる)

- [x] **Step 2: コミット依頼** 対象ファイルを提示し、ユーザーに `feat(claude): Claude Code 基盤(settings/CLAUDE.md/coder/commands/modern-web-guidance)を構築` でのコミットを依頼する(git commit はユーザー特権。README の Global Constraints 参照)
...
- 2026-07-19 実行。計画との差異は以下のみで、成果物の内容は全て計画どおり:
  - `.claude/settings.json` の Write が Claude Code の permission classifier に一度拒否された(allow/deny 権限を含む設定ファイルをエージェント自身が書き込む操作が制限対象)。ユーザーに確認し、再試行の許可を得て作成した。内容は phase 文書 Task 1 の JSON と同一
- coder.md / start_dev.md / create-next-prd.md は Read で確認の結果、移植元固有の記述が無く無修正コピー。model 固定は D10 どおり維持
- modern-web-guidance は 138 ファイルを丸ごとコピーし `diff -r` で移植元と同一を確認。symlink は移植元と同じ相対パス形式(`../../.agents/skills/modern-web-guidance`)
- 両 app の `npm run quality` を実行し exit 0・警告 0 を確認済み(本フェーズはコード変更なし)
- **ユーザーへの依頼(未完了の受入基準)**: 新しいセッションを起動し、CLAUDE.md が読み込まれて日本語応答・ワークフロー指示が効いていることを簡易確認する(Phase 7 開始セッションの冒頭で自然に確認可能)
