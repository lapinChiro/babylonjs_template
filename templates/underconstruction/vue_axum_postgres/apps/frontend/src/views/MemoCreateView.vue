...
async function handleSubmit() {
  ...
  const success = await memoStore.createMemo(memoData)
...
function handleCancel() {
  if (title.value || content.value) {
    if (!confirm('入力した内容は破棄されます。よろしいですか？')) {
      return
    }
  }
  router.push('/memos')
...
<template>
  <div class="memo-create-container">
    <div class="memo-create-header">
      <h1>メモ新規作成</h1>
    </div>

    <form @submit.prevent="handleSubmit" class="memo-form">
