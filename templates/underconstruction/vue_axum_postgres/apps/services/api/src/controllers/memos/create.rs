...
async fn inner(
    ...
    execute_db(&pg_pool, title, content, user.user_uuid).await?;
...
async fn execute_db(pg_pool: &sqlx::PgPool, title: &str, content: &str, user_uuid: Uuid) -> Result<DbOutput, sqlx::Error> {
