...
DB schema に新しいエンティティテーブルを追加するとき（migration で `CREATE TABLE` するとき）、または repository にエンティティの削除操作を実装・変更するとき。

## 背景

エンティティテーブルの削除方式が論理削除（`is_deleted` フラグ）と物理削除（`DELETE`）で混在し、その選定根拠が一部テーブルで未文書化のまま割れていた。実態は「物理削除が既定・論理削除が例外」という慣習がコードベース全体で成立していたが、規約として言語化されていなかったため、個々の物理削除テーブルを見ると「なぜ物理か」の根拠が無く恣意的に見えた。本規約はこの非対称を「物理=既定（根拠不要）・論理=例外（根拠必須）」と固定して drift の再発を防ぐ。[`tz-storage-convention`](tz-storage-convention.md) / [`uuid-primary-key-generation`](uuid-primary-key-generation.md) の兄弟に当たる「テーブル設計の選定規約」である。
...
判定軸（監査追跡の要否）は**人間の判断に依存し機械判定できない**ため、`uuid-primary-key-generation` の introspection メタテストのような自動 enforcement は設けない。「分類の正しさ」は検証不能であり、「例外の根拠コメント存在」の機械検査も安定な小集合（例外は現状 2 件）に対し over-engineering（YAGNI）。enforcement は本 rule の path-scope 自動ロード + review に委ねる。

この非対称（`uuid-primary-key-generation` はメタテスト有・本規約は無）は**抽象レベルの実際の差異**（機械判定可能な普遍規約 vs 判断依存の選定軸）を反映したものであり、恣意的非対称ではない。
...
- [`tz-storage-convention`](tz-storage-convention.md) / [`uuid-primary-key-generation`](uuid-primary-key-generation.md)（兄弟のテーブル設計選定規約。後者は enforcement 非対称の対）
