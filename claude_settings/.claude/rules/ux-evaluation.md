...
- 用語はユーザー（社内ロール）の語彙に合わせる。同種の操作・表示は既存画面と同じパターン・同じ component を使う（独自実装で対称を崩さない）。
...
- **レイアウト幅・縦 spacing（8pt）・画面下余白**は [`frontend-layout-spacing.md`](frontend-layout-spacing.md) が担う。本 rule は重複して扱わない。
...
- [`frontend-layout-spacing.md`](frontend-layout-spacing.md)（レイアウト spacing。責務分界）
